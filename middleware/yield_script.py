...
def main():
    ...
    parser.add_argument(
        "--input_path", type=str, default=None, help="Path to csv input",
    )
    args = parser.parse_args()

    input_file = args.input_path
    report = pd.read_csv(input_file)
    report[["Season", "State/Crop/District"]] = report[
        ["Season", "State/Crop/District"]
    ].fillna("")
    ...
    for index, row in report.iterrows():
        if index + 1 < len(report):
            if (
                row["Season"] == ""
                and "Total" not in row["State/Crop/District"]
                and report.loc[index + 1]["Season"] == ""
            ):
                # New State !
